...
async function listFiles(input) {
  if (values.file)
    return [resolve(process.cwd(), values.file)];
  if (!input)
    throw new Error("Missing --file or --input");
  const inputDir = resolve(process.cwd(), input);
  const entries = await readdir(inputDir);
  return entries.filter((entry) => entry.endsWith(".rackula.yaml")).map((entry) => resolve(inputDir, entry));
}
...
function validateLayout(doc, file) {
  ...
  if (!doc || typeof doc !== "object")
    errors2.push("document must be a YAML object");
  if (doc?.rack)
    errors2.push("use top-level racks array, not legacy rack object");
  if (!doc?.version)
    errors2.push("missing version");
  if (!doc?.name)
    errors2.push("missing name");
  if (!Array.isArray(doc?.racks))
    errors2.push("missing racks array");
  if (!Array.isArray(doc?.device_types))
    errors2.push("missing device_types array");
  if (!doc?.settings || typeof doc.settings !== "object")
    errors2.push("missing settings object");
  for (const type of deviceTypes) {
    if (!type?.slug) {
      errors2.push("device_type missing slug");
      continue;
    }
    if (typeBySlug.has(type.slug))
      errors2.push(`duplicate device_type slug ${type.slug}`);
    typeBySlug.set(type.slug, type);
    if (typeof type.u_height !== "number" || type.u_height < 0.5)
  ...
  for (const rack of racks) {
    ...
    for (const device of asArray(rack?.devices)) {
      ...
      if (!device?.id)
        errors2.push(`${rack?.id ?? "rack"}: device missing id`);
      if (!device?.device_type)
        errors2.push(`${deviceId}: missing device_type`);
      if (!type)
        errors2.push(`${deviceId}: missing device_type ${device?.device_type}`);
      if (!device?.face)
      ...
      if (type && typeof device?.position === "number" && !isContainerChild) {
        ...
        if (end > rackUnits)
          errors2.push(`${deviceId}: does not fit in rack`);
        occupied.push({
          id: deviceId,
    ...
    for (let i = 0;i < occupied.length; i += 1) {
      for (let j = i + 1;j < occupied.length; j += 1) {
        ...
        if (overlaps && slotConflicts)
          errors2.push(`collision: ${a.id} and ${b.id}`);
      }
    }
  }
  for (const connection of asArray(doc?.connections)) {
    ...
    if (connection.from || connection.to || connection.source || connection.target) {
      errors2.push(`${id}: connections must use a_port_id and b_port_id, not from/to/source/target`);
    }
    if (!connection?.a_port_id)
      errors2.push(`${id}: missing a_port_id`);
    if (!connection?.b_port_id)
      errors2.push(`${id}: missing b_port_id`);
    if (connection?.a_port_id && !portIds.has(connection.a_port_id))
      errors2.push(`${id}: missing a_port_id port ${connection.a_port_id}`);
    if (connection?.b_port_id && !portIds.has(connection.b_port_id))
      errors2.push(`${id}: missing b_port_id port ${connection.b_port_id}`);
    if (connection?.a_port_id && connection.a_port_id === connection.b_port_id)
      errors2.push(`${id}: a_port_id and b_port_id must differ`);
    if (connection?.color && !hexColor.test(connection.color))
      errors2.push(`${id}: color must be #RRGGBB`);
  }
  if (errors2.length > 0) {
